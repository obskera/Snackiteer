...
> **Tip:** Set `ENABLE_TESTS=false` in a root `.env` file to skip test commands during rapid prototyping. Re-enable with `ENABLE_TESTS=true` (or delete `.env`) before opening a PR.
