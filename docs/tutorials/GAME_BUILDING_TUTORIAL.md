...
> **Tip:** During rapid prototyping you can skip test commands by setting `ENABLE_TESTS=false` in a root `.env` file. Re-enable with `ENABLE_TESTS=true` (or delete `.env`) before opening a PR. See [../CONTRIBUTING.md](../CONTRIBUTING.md) for details.
