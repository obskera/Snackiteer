...
### Tile matrix room template system (`src/logic/tileMatrix/`)

- Text-based room authoring via space-delimited tile codes and entity overlay matrices.
- Intermediate `RoomTemplate` format (`um-room-template-v1`) with converters to `TileMapPlacementPayload` and worldgen input.
- In-memory template registry for library management.
- Tool UI in `src/components/examples/TileMatrixEditorTool.tsx`.
- Spec: [docs/tools/TILE_MATRIX_SPEC.md](tools/TILE_MATRIX_SPEC.md). Guide: [docs/tools/TILE_MATRIX_EDITOR.md](tools/TILE_MATRIX_EDITOR.md).
...
All test commands can be disabled via `ENABLE_TESTS=false` (see [CONTRIBUTING.md](CONTRIBUTING.md)). This is useful for AI workflows or rapid prototyping. CI always runs with tests enabled.
