...
- [ ] Dialogue system suite: bubble dialogue, dynamic text boxes, typing/typewriter text effect, and talking-head dialogue UI.
    - Deliverables:
        - speech bubble component (anchored to entity, auto-size, tail pointer).
        - dynamic text box primitive (resizable, themed, portrait-ready).
        - typewriter text renderer (configurable speed, skip-on-input, punctuation pauses).
        - talking-head dialogue panel (portrait + name plate + text box + advance/choice controls).
        - integration hooks for branching dialogue data from `dialogue-social-pack`.
        - focused tests and docs/usage snippets.
