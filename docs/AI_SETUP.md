...
## Reducing AI context usage (ENABLE_TESTS toggle)

When using AI tools to work on this repo, you can skip test execution to save tokens and context window space.

**To disable tests for AI sessions:**
...
# — or delete .env (tests default to enabled when unset)
```

This causes all `test:*` and `quality:*` npm scripts to exit immediately with a skip message instead of invoking vitest. Non-test scripts (`lint`, `build`, `content:validate:*`, standalone CLI tools) are unaffected.

### Why this helps AI workflows

- Avoids ~179 test files of output in AI context, freeing tokens for application code.
- AI can still run `npm run lint` and `npm run build` for validation.
- No risk to CI — tests are always enforced in GitHub Actions (`ENABLE_TESTS=true` is hardcoded).
- Re-enabling is instant: flip the env var back or delete `.env`.

### Updated validation bundles

**With tests enabled (default / pre-PR):**

- Focused change: `npm run test:run -- src/tests/<target>.test.ts` then `npm run lint`
- Cross-cutting change: `npm run lint` then `npm run test:run`
- Release readiness: `npm run lint && npm run test:run && npm run test:coverage:strict`

**With tests disabled (AI context-saving mode):**

- Focused change: `npm run lint`
- Cross-cutting change: `npm run lint && npm run build`
- Before PR (re-enable first): `echo "ENABLE_TESTS=true" > .env && npm run lint && npm run test:run`
