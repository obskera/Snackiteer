...
### Opting out of tests (ENABLE_TESTS toggle)

All `test:*` and `quality:*` npm scripts respect the `ENABLE_TESTS` environment variable.

| Setting | Behavior |
|---------|----------|
| Unset (default) | Tests run normally — current behavior preserved |
| `ENABLE_TESTS=true` | Tests run normally (explicit opt-in) |
| `ENABLE_TESTS=false` | All test/quality commands exit immediately with a skip message |
...
# — or simply delete .env (tests default to enabled when unset)

# If using shell export:
export ENABLE_TESTS=true
# — or: unset ENABLE_TESTS
```

What this **does not** skip:

- `npm run lint` — always runs
- `npm run build` — always runs
- `npm run content:validate:*` — standalone CLI scripts, not vitest
- `npm run balance:simulate`, `npm run accessibility:qa`, etc. — standalone CLI tools

**When to re-enable:** before opening a PR, before running `release:verify`, or whenever you want full confidence in your changes.

**Why skip tests?**

- **AI workflows:** reduces token/context usage significantly (~179 test files' worth of output avoided), letting the AI focus on application code.
- **Rapid prototyping:** lighter feedback loop when exploring ideas.
- **Preference:** some contributors simply prefer a leaner dev cycle during early work.

Tests stay in the repo and CI always runs them (`ENABLE_TESTS=true` is hardcoded in `.github/workflows/ci.yml`), so there is zero risk of regressions slipping through.
...
- [ ] Tests pass (`npm run test:run`) — re-enable tests first if `ENABLE_TESTS=false`
