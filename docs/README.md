...
Set `ENABLE_TESTS=false` in a `.env` file to skip all test and quality-gate commands:

```bash
echo "ENABLE_TESTS=false" > .env   # disable tests
echo "ENABLE_TESTS=true" > .env    # re-enable (or delete .env)
```

Useful for AI-assisted workflows (saves context/tokens) or lighter prototyping loops. Lint and build always run regardless. See [CONTRIBUTING.md](CONTRIBUTING.md) for full details.
